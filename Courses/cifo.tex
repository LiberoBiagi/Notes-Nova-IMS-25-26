...
\begin{itemize}
    \item \textbf{Problem} \ra Task to be solved automatically (finde the maximum number in a list)
...
\begin{itemize}
    ...
    \item We need computational intelligence because traditional methods in many case fail, usually on the first step. We can have many problems where imaging solutions or algorithms it's either impossible or very hard for human being to imagine a solution. This is the case of \textbf{complex problems}
\end{itemize}
...
To solve these kind of problems we can use an optimization algorithm, a machine that is able to output multiple solutions (iterative machine). 
...
To find the solution we will use some optimization algorithm, by definition an optimization algorithm is an iterative methos that at the end of each iteration returns a solution \begin{math}
    \in 
...
\begin{enumerate}
    \item It is impossible to have an algorithm that is better than all the others an all possible problems (no super algorithm can exist).
...
\textbf{Local otpimum} \ra given an optimization problem (S, f) a local optimum is a solution x such that $\forall y \in N(x), f(x)$ is better than $f(y)$. By definition hill climbing will return a local optimum with no guarantee that it will also be global. We can find the global optimum by expanding the radius or by trying more initializations

\vspace{10pt}

In pseudo code (general case)
...
Tabu search can help the search by using memory, avoid the already checked points. 
...
\begin{itemize}
    \item X \ra all solutions produced ordered ny their neighborhood
...
With the fitness landscape we can find the difficult of the problem. A very smooth one is usually of a simple problem while a very obscillating one will be probably more difficult.


\subsection{Fitness landscape}

In the last class we saw what is a fitness landscape. The plot in the 2-D setting is obtain by plotting on the axis
...
Given an optimization problem (S, f) and a neighborhood N, a solution $x \in S$ is called a local otpimum if $\forall y \in N(x)$ f(y) is worse than f(x). By construction hill climbing will return a local optimum with no guarantee that it will also be a global otpimum.
...
Here hill climbing will arrive at the global otpimum.

\vspace{10pt}

If we want to look for the number of 1 in the binary code of i we can encounter problems and stay stuck in a local otpimum without being able to find the global one. 
\vspace{10pt}
...
\begin{enumerate}
    ...
    \item As sequences of values, a string of bits of length N. The array will have a pair cost, profit. We use 0s and 1s to "1-hot encoding" them. 
